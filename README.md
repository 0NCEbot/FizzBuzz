...
- Python's `and` is `&&` in Java
- Python's `for i in (...):` vs Java's `for (int i = _; terminating condition; increment) {...}`
- Python's `if __name__ == __main__:` vs Java's `public static void main(String[] args) {...}` (right?)
- Python's `elif (cond):` is `else if (cond) {}` in Java
- Variable types must be specified in Java
...
think about how they differ and which might be best.
  * Solution: pass `args` to `FizzBuzz.main()` — this is the same `args` from `Main.main`
